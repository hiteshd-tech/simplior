...
function iqfix_wpcf7_recaptcha_enqueue_scripts() {
	...
	$source = ( in_array( $source, array( 'google.com', 'recaptcha.net' ) ) ) ? $source : 'google.com';
	...
	wp_localize_script( 'google-recaptcha', 'wpcf7recaptchafix', array(
...
document.addEventListener( 'wpcf7spam', function( event ) {
	...
	Array.prototype.forEach.call( wpcf7forms, function( form ) {
		...
		if( '' === response.value ) {
			...
			wpcf7.notValidTip( recaptchaWrapper, wpcf7recaptchafix.recaptcha_empty );
		}
	} );
} );
</script>
...
function iqfix_wpcf7_recaptcha_form_tag_handler( $tag ) {
	...
	$html = sprintf( '<div class="wpcf7-form-control-wrap recaptcha">%s</div>', $html );
